    ...
    <div class="flex mt-5">
        ...
        <div class="flex-1 h-1/2 w-1/3 px-5 mx-auto">
            <div class="border-4 border-gray-700 text-xl p-5 text-center">
                ...
                <span v-if="title_for_ward_map === 'brexit'"><strong>Gloucester Referendum Results by Ward</strong> (Leave
                    Vote %)</span>
                <span v-if="title_for_ward_map === 'deprivation'"><strong>Gloucester: <a
                            href="https://data.cdrc.ac.uk/dataset/index-multiple-deprivation-imd"
                            class="text-blue-500">Index for Multiple Deprivation</a></strong></span>
            </div>
            ...
            <div class="flex justify-center mt-2 border-gray-500">
                ...
                <button class="p-2 bg-gray-100 border-2 border-black mx-2 text-xl "
                    @click="apply_brexit_layer_to_map"><strong>Referendum results</strong></button>
                <button class="p-2 bg-gray-100 border-2 border-black mx-2 text-xl  " @click="init_elections_map"><strong>Council
                        Election Results</strong></button>
                <button class="p-2 bg-gray-100 border-2 border-black mx-2 text-xl"
                    @click="init_poverty_map"><strong>Relative Deprivation</strong></button>
                <button class="p-2 bg-gray-100 border-2 border-black mx-2 text-xl"
                    @click="neutralise"><strong>Neutralise Map Colours</strong></button>
            </div>
        </div>
            ...
            <div class="flex flex-row">
                <div id="council-election-results" v-if="ward_election_datacard" class="flex-1">
                    ...
                    <div class="mt-5 text-center border-l border-b border-t border-gray-700 text-xl py-2">
                        <strong>Last Council Election: </strong>
                        {{ new Date(ward_election_datacard['PreviousElection']).toLocaleDateString('en - UK', { day: '2 - digit', month: 'short', year: 'numeric' }) }}
                    </div>
                        ...
                        <div class="overflow-auto max-h-1/2" style="height: 50vh;">
                            <div v-for="(result, key) in ward_election_datacard['Results']" :key="key">
                                <div :class="{
                                    ...
                                    <strong class="pl-3">Outcome:</strong> {{ result[key + 1].Outcome }} ({{ result[key
                                        + 1].Percentage
                                    }})<br>
                                </div>
                                <hr>
                ...
                <div id="brexit-referendum-results" class="flex-1">
                    <div v-if="ward_election_datacard" :class="{
                        ...
                        <div
                            class="mt-5 text-center border-l border-b border-t border-r border-gray-700 text-xl py-2 bg-white">
                        ...
                        <div class="border-l border-gray-700 pl-3 py-2 bg-white border-r">
                            ...
                            <strong>Electorate: </strong>{{ brexit_election_datacard['electorate'] }} eligible
                            voters<br>
                            <strong>Votes Cast: </strong>{{ brexit_election_datacard['total_votes'] }} <br>
                        </div>
                        <hr class="border-t-3 border-gray-500">
                        <div
                            class="text-center bg-white border-l border-b border-r border-gray-700 text-xl text-center mr-0">
                        ...
                        <div
                            class="mt-5 text-center border-l border-b border-t border-r border-b border-gray-700 text-xl py-2 bg-white">
                        ...
                        <div @click="show_areas" class="border-l border-b border-gray-700 pl-3 py-2 bg-white border-r">
                            ...
                            <strong>Mean Deprivation Ranking: </strong>{{
                                Math.round(imd_ward_datacard.mean_absolute_deprivation_ranking * 10) / 10 }}%<br>
                            <i class="text-gray-700">NOTE: Lower number indicates a higher relative deprivation</i><br>
                        </div>
                        <div v-if="show_deprivation_areas">
                            <div v-for="(lsoa_info, key) in imd_ward_datacard['other_area_info']" :key="key"
                                class="pl-3 border-l border-b border-r border-gray-700 bg-gray-100">
...
export default {
    ...
    methods: {
        ...
        async fetch_ward_results_by_name(ward_name = "") {
            fetch(`/election_results_oldham.json`)//?ward_name=${ward_name}`)
        ...
        async init_elections_map() {
            ...
            const response = await fetch('/election_results_oldham.json');
            const election_data = await response.json();
            // Define the projection
            this.projection = d3.geoMercator()
                .center([-2.16265,53.53345])
                .scale(245000)
                .translate([400, 400]);

            this.path = d3.geoPath().projection(this.projection);

            await d3.json("/oldham_west_wards.geojson")
                .then(data => {
                    const svg = d3.select(this.$refs.map);
                    ...
                    function get_colour(d) {
                        ...
                        if (parties_elected.length === 1) {
                            switch (parties_elected[0]) {
                                case 'Labour': //needs to check substring
                                    return lab_colour;
